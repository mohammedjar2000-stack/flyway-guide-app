...
function CountBadge({ value, compact }: { value: number; compact?: boolean }) {
  return (
    <span className={`${compact ? 'inline-flex' : 'hidden md:inline-flex ms-auto'} shrink-0 min-w-[1.15rem] justify-center tabular-nums text-[10px] font-semibold leading-none tracking-tight text-slate-600`}>
...
export default function CategoryFilterBar({ selected, onChange, counts, total = 0, layout = 'rail' }: CategoryFilterBarProps) {
  ...
  const chipClass = grid
    ? 'flex flex-col items-center justify-center gap-1.5 min-h-[5.75rem] rounded-2xl px-2 py-3 text-[12px] font-medium leading-tight cursor-pointer select-none border transition-all duration-150 text-center'
    : chips
  ...
  const idle = grid || chips
    ? 'bg-slate-50 text-slate-800 border-slate-200 hover:bg-white hover:border-brand-400/50'
  ...
  const items = (
    <>
      ...
      >
        ...
        <span className={grid || chips ? 'inline truncate text-slate-800' : 'hidden md:inline truncate'}>الكل</span>
      ...
      {FILTER_BAR_GROUPS.map((group) => {
        ...
        return (
          ...
          >
            ...
            <span className={grid || chips ? 'inline truncate text-center text-slate-800' : 'hidden md:inline truncate text-right'}>
              {group.id === 'telecom' ? (
